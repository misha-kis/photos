...
impl ImageLoader {
    ...
    pub(crate) async fn get_unique_face_thumbnails(&mut self) -> Result<Vec<FaceThumbnail>> {
        ...
        let face_detections = self.db_worker.lock().await.get_unique_face_detections().await?;
        ...
        for face_detection in face_detections.into_iter() {
            let full_image = self.get_full_image(face_detection.image_id).await.context("Failed to get full image")?;
            full_images.push((full_image, face_detection.bounding_box, face_detection.detection_id, face_detection.face_id));
        }
        tracing::debug!("got {} full images", full_images.len());
        let face_thumbnails = full_images.into_par_iter().map(|(full_image, bounding_box, detection_id, face_id)| {
