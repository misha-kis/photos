...
impl<ItemId: Copy + std::hash::Hash + Eq, ItemData> DynamicGrid<ItemId, ItemData> {
        ...
        FClick: FnMut(usize),
    {
        ...
        let n_columns = ((available_width + spacing) / (self.desired_item_size + spacing))
            .floor()
            .max(1.0) as usize;
        ...
        let total_rows = items.len().div_ceil(n_columns);
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let mut start_index = start_row * n_columns;
                let mut end_index = (end_row * n_columns).min(items.len());
                ...
                let mut i = 0;
                for chunk in items.chunks(n_columns) {
                    ui.horizontal(|ui| {
